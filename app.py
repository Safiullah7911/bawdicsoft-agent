"""
BawdicSoft AI Sales Agent — IP + Email dual tracking + CORS enabled + Lead forward.
...
FRONTEND_LEAD_URL = os.environ.get(
    "FRONTEND_LEAD_URL", "https://www.bawdicsoft.com/api/tracking/lead"
)
...
def maybe_capture_lead(session_id, visitor_text, history, page, ip):
    ...
    if lead.get("email") and lead.get("name"):
        fire_lead_webhooks(session_id, lead, ip, page)


def fire_lead_webhooks(session_id, lead, ip, page="/"):
    """Save lead locally + forward to frontend (Airtable + Email)."""
    print(f"[LEAD CAPTURED] session={session_id} ip={ip} page={page} lead={lead}")
    ...
    payload = {
        ...
        "email": email,
        "name": lead.get("name", ""),
        "interest": clean_interest(lead.get("project", "")),
        "ip": ip,
        "page": page,
        "source": "bawdicsoft-agent",
    }
    try:
        resp = requests.post(FRONTEND_LEAD_URL, json=payload, timeout=15)
        print(f"[FORWARD] status={resp.status_code} body={resp.text[:200]}")
    except Exception as e:
